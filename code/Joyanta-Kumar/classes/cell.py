...
class Cell:
    ...
    def draw(self, cellColor=clr.cell, wallColor=None, border=0, padding=0,):
        ...
        if wallColor:
            if self.walls["top"]:
                line(window, wallColor, (x, y), (x+size, y), 1)
            if self.walls["left"]:
                line(window, wallColor, (x, y), (x, y+size), 1)
            if self.walls["bottom"]:
                line(window, wallColor, (x, y+size), (x+size, y+size), 1)
            if self.walls["right"]:
                line(window, wallColor, (x+size, y), (x+size, y+size), 1)
